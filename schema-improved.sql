-- ============================================================================
-- IMPROVED SCHEMA - Performance, Security, and Data Integrity Enhancements
-- ============================================================================
...
-- 3. Indexes - Added composite index for common query patterns
CREATE INDEX IF NOT EXISTS idx_user_roles_user_id ON public.user_roles (user_id);
CREATE INDEX IF NOT EXISTS idx_user_roles_role ON public.user_roles (role);
-- Composite index for role checks (used frequently in RLS policies)
...
-- Optimized: Uses index on (user_id, role)
...
-- Optimized: Single query to check both roles instead of two separate queries
CREATE OR REPLACE FUNCTION public.grant_role(p_target uuid, p_role app_user_role) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  caller_roles app_user_role[];
BEGIN
  -- Get all caller roles in a single query
  SELECT ARRAY_AGG(role) INTO caller_roles
  FROM public.user_roles
  WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin');

  -- Authorization: super_admin or admin required
  IF caller_roles IS NULL OR array_length(caller_roles, 1) IS NULL THEN
    RAISE EXCEPTION 'not authorized to grant roles';
  END IF;

  -- If caller is admin (and not super), restrict which roles they can grant
  IF NOT ('super_admin' = ANY(caller_roles)) AND ('admin' = ANY(caller_roles)) THEN
    IF p_role NOT IN ('sales', 'sales_support') THEN
      RAISE EXCEPTION 'admin may only grant sales or sales_support roles';
    END IF;
  END IF;

  -- Insert role if not exists (using INSERT ... ON CONFLICT for better performance)
  INSERT INTO public.user_roles (user_id, role, granted_by)
  VALUES (p_target, p_role, auth.uid())
  ON CONFLICT (user_id, role) DO NOTHING;
END;
$$;

-- Optimized: Single query to check both roles
...
CREATE TABLE public.profiles (
  ...
  last_name text,
  email text,
  metabase_dashboard_id integer CHECK (metabase_dashboard_id IS NULL OR metabase_dashboard_id > 0),
...
CREATE INDEX IF NOT EXISTS idx_profiles_metabase_id ON public.profiles (metabase_dashboard_id) WHERE metabase_dashboard_id IS NOT NULL;
...
FOR EACH ROW 
WHEN (OLD.* IS DISTINCT FROM NEW.*)  -- Only update if something actually changed
...
-- INSERT policy (FIXED: Added proper parentheses for logic)
...
CREATE POLICY "profiles_delete_admins" ON public.profiles
  ...
  USING (
    public.is_user_in_role('super_admin')
    OR (public.is_user_in_role('admin') AND NOT public.user_has_role(id, 'super_admin'))
  );

-- ============================================================================
-- OPTIMIZED: get_profiles_with_role() - Uses DISTINCT ON instead of LATERAL JOIN
-- ============================================================================
...
LANGUAGE sql
STABLE
SECURITY DEFINER  -- Added for RLS bypass when needed
AS $$
  ...
  ORDER BY p.id, ur.granted_at DESC NULLS LAST, p.created_at DESC;
$$;

-- Grant execute permission
GRANT EXECUTE ON FUNCTION public.get_profiles_with_role() TO authenticated, supabase_auth_admin;
REVOKE EXECUTE ON FUNCTION public.get_profiles_with_role() FROM anon, public;
...
CREATE INDEX idx_sales_summaries_user_id ON public.sales_summaries(user_id);
CREATE INDEX idx_sales_summaries_created_at ON public.sales_summaries(created_at DESC);
-- Composite index for common query: user + date range
CREATE INDEX idx_sales_summaries_user_created ON public.sales_summaries(user_id, created_at DESC);

-- Trigger to update updated_at on change
CREATE TRIGGER sales_summaries_set_updated_at
BEFORE UPDATE ON public.sales_summaries
FOR EACH ROW 
WHEN (OLD.* IS DISTINCT FROM NEW.*)  -- Only update if something actually changed
EXECUTE FUNCTION public.trigger_set_updated_at();
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "sales_summaries_select_by_role_owner" ON public.sales_summaries;
DROP POLICY IF EXISTS "sales_summaries_insert_owner_or_admin" ON public.sales_summaries;
DROP POLICY IF EXISTS "sales_summaries_update_owner_or_admin" ON public.sales_summaries;
DROP POLICY IF EXISTS "sales_summaries_delete_admins" ON public.sales_summaries;
...
CREATE POLICY "sales_summaries_update_owner_or_admin" ON public.sales_summaries
  ...
  USING (
    public.is_user_in_role('super_admin')
    OR (public.is_user_in_role('admin') AND NOT public.user_has_role(user_id, 'super_admin'))
    OR (user_id = auth.uid())
  )
  WITH CHECK (
    public.is_user_in_role('super_admin')
